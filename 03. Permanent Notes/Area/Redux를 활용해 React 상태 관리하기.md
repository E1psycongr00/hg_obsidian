...
[[03. Permanent Notes/Area/React Context API와 Reducer로 상태 관리하기]]를 활용하면 [[03. Permanent Notes/Area/Props Driling]] 없이 컴포넌트가 바로 상태를 활용하고 동기화가 가능하다. 매우 좋은 방법 중 하나지만 만약 상태가 복잡해 매번 이렇게 컴포넌트를 관리해야 한다면 타입 선언에 많은 코드를 짜야 하기 때문에 힘들다.
## 해결 방안
### Redux

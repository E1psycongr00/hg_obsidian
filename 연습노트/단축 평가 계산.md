...
단축평가를 사용한 버전과 안사용한 버전 둘 다 살펴보자
...
    user && user.name || (() => { throw new Error("User not found"); })();
    return user.name;
}
```

조금 극단적인 단축 평가 사용 같기는 한데 이런식으로 사용할 수 있다.
isLogin이 참이면 상관 없지만 isLogin이 거짓이면 || 우측의 예외를 실행시킨다. 이것이 가능한 이유는 즉시 실행 함수 (()=>{}) IIFE
## 질문 & 확장

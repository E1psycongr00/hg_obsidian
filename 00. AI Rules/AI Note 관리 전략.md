...
AI는 다음 단계별 프로세스에 따라 각 유형의 노트 생성 및 초기 관리를 수행합니다:

1. **기본 규칙 확인**: [[AI 옵시디언 기본 규칙.md]]의 해당 노트 유형 규칙 준수
2. **상황별 프롬프트 적용**: 아래 정의된 노트 유형별 구체적 생성 프롬프트 실행
3. **자가검증 수행**: 생성 완료 후 품질 체크리스트 점검
4. **연결 관계 구축**: 관련 MOC 및 기존 노트와의 링크 설정

## 3. AI 노트 관리 핵심 원칙

AI는 다음 7가지 핵심 원칙에 따라 사용자님의 Obsidian 볼트 내 노트 관리를 지원해야 합니다:

1. **기본 규칙 우선 준수**: [[AI 옵시디언 기본 규칙.md]]의 모든 규칙을 최우선으로 준수
2. **노트 유형별 구조 엄격 준수**: 각 `note-type`별 Frontmatter, 제목 규칙, 내용 구조 정확 적용
3. **날짜 정확성**: 생성일은 반드시 실제 생성 당일 날짜 사용 (YYYY-MM-DD 형식)
4. **폴더 위치 정확성**: 각 노트 유형에 맞는 정확한 폴더에 저장
5. **MOC 연결 자동화**: 신규 노트 생성 시 관련 MOC에 링크 추가 제안
6. **태그 일관성**: 정의된 태그 지침에 따른 일관된 태그 적용
7. **품질 검증**: 생성 후 반드시 자가검증 체크리스트로 품질 확인
...
#### 1단계: 노트 생성
1. **노트 유형 결정**: 목적에 맞는 적절한 `note-type` 선택
2. **기본 규칙 확인**: [[AI 옵시디언 기본 규칙.md]] 해당 섹션 참조
3. **프롬프트 적용**: 위 정의된 노트 유형별 AI 프롬프트 실행
4. **자가검증**: 5단계 체크리스트로 품질 확인

#### 2단계: 연결 관계 구축
1. **관련 MOC 식별**: 새 노트가 속할 MOC 찾기
2. **기존 노트 연결**: 관련된 기존 노트들과 링크 생성
3. **태그 일관성**: 기존 노트들과 일관된 태그 적용
4. **폴더 정리**: 필요시 적절한 폴더로 이동

#### 3단계: 품질 관리
1. **5분 규칙 적용**: Common Notes의 직관적 이해 가능성 검증
2. **단일 문제 집중**: Solution Notes의 범위 적절성 확인
3. **내용 완성도**: 각 노트 유형의 목적에 맞는 내용 충실도 점검
4. **아카이빙 고려**: 완성된 노트의 영구 보관 위치 검토
...
#### 개선 영역
- **프롬프트 정확성**: 실제 사용 결과에 따른 프롬프트 세부 조정
- **체크리스트 실용성**: 자가검증 항목의 효과성 평가 및 개선
- **워크플로우 효율성**: 노트 생성부터 관리까지의 전체 프로세스 최적화

...
-   **AI 프롬프트**: "새로운 '일일 노트' 생성 요청 시:
    ...
    6.  **저장 폴더**: 사용자의 Obsidian '일일 노트' 플러그인 설정에 지정된 폴더를 우선적으로 따릅니다. 해당 설정이 없거나 비활성화된 경우, 사용자에게 기본 저장 위치(예: Obsidian 볼트 최상위 `/` 또는 `Journal/YYYY/MM`과 같은 특정 폴더 구조)를 확인받아 해당 위치에 생성합니다."
...
-   **AI 프롬프트**: "특정 프로젝트에 대한 '프로젝트 노트' 생성 요청 시 (예: '개인 블로그 개발 프로젝트 노트'):
    ...
    5.  **저장 폴더**: `09. PARA/1.Projects/[프로젝트 이름 폴더]`. 해당 폴더가 존재하지 않을 경우, 사용자에게 생성 여부를 확인한 후 생성하고 노트를 저장합니다.
    6.  **기본 구조**:
        ```markdown
        ## 목표
        - 
...
-   **AI 프롬프트**: "특정 주제에 대한 'MOC 노트' 생성 요청 시 (예: '프로그래밍 언어 MOC 노트'):
    ...
    4.  `created`: `YYYY-MM-DD` 형식으로 설정합니다.
    5.  `cssclasses: -dashboard` (템플릿 또는 사용자 지정에 따라 적용)으로 설정합니다. (`Note Guides.md`의 MOC 노트 템플릿 정의와 일치시킵니다.)
    6.  **저장 폴더**: `02. MOC`
    7.  **기본 구조**:
        ```markdown
        ## [하위 주제 1 또는 링크 그룹 1]

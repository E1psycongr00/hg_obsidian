...
AI는 다음 프롬프트를 참고하여 각 유형의 노트 생성 및 초기 관리를 수행합니다. **모든 frontmatter 필드와 값은 `01. Guides/Note Guides.md`에 정의된 해당 노트 유형의 규칙을 반드시 엄격하게 준수해야 합니다.**
...
-   **AI 프롬프트**: "새로운 '일일 노트' 생성 요청 시:
    ...
    5.  `tags`: `daily` 를 포함합니다. (그 외 `01. Guides/Note Guides.md`에서 정의된 태그가 있다면 포함)
...
-   **AI 프롬프트**: "문제 해결을 위한 '솔루션 노트' 생성 요청 시 (예: '웹사이트 로딩 속도 문제 해결 노트'):
    ...
    6.  `area-reason: ""` (사용자가 관련 기술 분야나 문제 영역을 명시할 수 있도록 비워둡니다. `01. Guides/Note Guides.md` 참조).
...
-   **AI 프롬프트**: "학습 내용 정리를 위한 '공부 노트' 생성 요청 시 (예: '머신러닝 기본 개념 정리 노트'):
    ...
    4.  `created`: **반드시 실제 생성 당일의 날짜**를 YYYY-MM-DD 형식으로 설정합니다. (예: 오늘이 2025년 1월 10일이면 `2025-01-10`)
...
-   **AI 프롬프트**: "특정 프로젝트에 대한 '프로젝트 노트' 생성 요청 시 (예: '개인 블로그 개발 프로젝트 노트'):
    ...
    4.  `created`: **반드시 실제 생성 당일의 날짜**를 YYYY-MM-DD 형식으로 설정합니다. (예: 오늘이 2025년 1월 10일이면 `2025-01-10`)
    5.  **날짜 확인**: 노트 생성 전 현재 날짜를 확인하고 정확한 오늘 날짜를 사용합니다.
    5.  `status: 진행중` (또는 사용자가 지정한 초기 상태. `01. Guides/Note Guides.md`의 예시 참조)으로 설정합니다.
...
-   **AI 프롬프트**: "외부 자료(웹, 책, 논문 등)에 대한 '참고 노트' 생성 요청 시 (예: '클린 코드 책 요약 노트'):
    ...
    2.  사용자로부터 자료 제목과 자료 유형(`from` 필드 값: `VIDEO`, `Lecture`, `Book`, `Blog`, `Paper`, `Web` 중 하나. **`01. Guides/Note Guides.md`에 정의된 정확한 값 중 하나를 반드시 입력받습니다.**)을 반드시 입력받습니다.
    3.  `title`: `📚 [제공된 자료 제목]` 형식으로 설정합니다.
    4.  `note-type: REF` 으로 설정합니다.
    5.  `created`: **반드시 실제 생성 당일의 날짜**를 YYYY-MM-DD 형식으로 설정합니다. (예: 오늘이 2025년 1월 10일이면 `2025-01-10`)
...
-   **AI 프롬프트**: "코드 스니펫 또는 코드 학습 관련 '코드 노트' 생성 요청 시 (예: '파이썬 리스트 정렬 함수 코드 노트', 언어: 'python'):
    ...
    2.  사용자로부터 코드 주제와 프로그래밍 언어(`language` 필드 값, 예: `py`, `js`, `java`. **`01. Guides/Note Guides.md`에 명시된 형식 또는 사용자 지정 규칙을 따릅니다.**)를 입력받습니다.
    ...
    8.  `area-reason: ""` (사용자가 특정 기술 스택이나 프로젝트 영역을 명시할 수 있도록 비워둡니다. `01. Guides/Note Guides.md` 참조).
...
-   **AI 프롬프트**: "특정 주제에 대한 '가이드 노트' 생성 요청 시 (예: '옵시디언 초기 설정 가이드 노트'):
    ...
    4.  `tags`: `Guide` 를 포함합니다. (필요시 `01. Guides/Note Guides.md`에서 정의된 추가 태그 포함)
    5.  `created`: **반드시 실제 생성 당일의 날짜**를 YYYY-MM-DD 형식으로 설정합니다. (예: 오늘이 2025년 1월 10일이면 `2025-01-10`)
    6.  **날짜 확인**: 노트 생성 전 현재 날짜를 확인하고 정확한 오늘 날짜를 사용합니다.
    7.  **저장 폴더**: `01. Guides`
    7.  **기본 구조**:
...
-   **AI 프롬프트**: "특정 주제에 대한 'MOC 노트' 생성 요청 시 (예: '프로그래밍 언어 MOC 노트'):
    ...
    4.  `tags`: 해당 MOC 주제 관련 태그 및 `MOC` 를 포함합니다. (`01. Guides/Note Guides.md` 참조)
    5.  `created`: **반드시 실제 생성 당일의 날짜**를 YYYY-MM-DD 형식으로 설정합니다. (예: 오늘이 2025년 1월 10일이면 `2025-01-10`)
    6.  **날짜 확인**: 노트 생성 전 현재 날짜를 확인하고 정확한 오늘 날짜를 사용합니다.
    7.  `cssclasses: -dashboard` (템플릿 또는 사용자 지정에 따라 적용. `01. Guides/Note Guides.md`의 MOC 노트 템플릿 정의와 일치)으로 설정합니다.
    7.  **저장 폴더**: `02. MOC`
    8.  **기본 구조**:
        ```markdown
        ## [하위 주제 1 또는 링크 그룹 1]
...
**모든 노트 생성 완료 후, AI는 다음 검증을 반드시 수행해야 합니다:**

1. **📅 날짜 및 Frontmatter 검증**:
   - `created` 필드에 실제 오늘 날짜가 YYYY-MM-DD 형식으로 정확히 입력되었는지 확인
   - 해당 노트 유형의 모든 필수 frontmatter 필드가 누락 없이 포함되었는지 확인
   - `note-type` 필드가 생성한 노트의 실제 유형과 정확히 일치하는지 확인

2. **🔬 Solution Notes 특별 검증** (Solution Notes인 경우만):
   - "문제 정의" 섹션이 하나의 구체적이고 명확한 문제만 다루고 있는지 확인
   - 여러 관련 문제를 동시에 다루지 않고 있는지 확인
   - 추가 문제들이 "관련 문제" 섹션에 별도 노트 작성 예정으로 표시되었는지 확인

3. **📝 콘텐츠 품질 검증**:
   - `00. AI Rules/AI 옵시디언 마크다운 스타일 준수 가이드.md`의 자가검증 체크리스트 준수
   - 서술어 일관성, 헤더 구조, 콜아웃 배치 등 스타일 규칙 준수 확인

**검증 실패 시 즉시 수정 후 재검증을 수행해야 합니다.**
...
3.  **콘텐츠 스타일 일관성**:
    *   노트 내용 작성 지원 시, `00. AI Rules/AI 옵시디언 마크다운 스타일 준수 가이드.md`에 명시된 글쓰기 스타일과 규칙을 최대한 일관성 있게 따릅니다.
